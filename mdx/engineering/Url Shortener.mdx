...
A URL shortener is a tool that allows you to take a long URL and create a shorter, easier-to-remember
...
1. Easy to share: Long URLs can be difficult to share, especially on social media platforms like
...
2. Track clicks: Often we require additional analytics that allow you to track how many clicks your
...
3. Save space: Shortened URLs take up less space than long URLs, which can be especially important
...
4. Hide the original URL: Shortened URLs can be used to hide the original URL, which can be useful
in situations where the original URL is long, unappealing, or contains sensitive information.
5. Redirect traffic: URL shorteners can be used to redirect traffic to different landing pages or
...
CREATE TABLE url_shortener (
  ...
  clicks INTEGER NOT NULL DEFAULT 0,
	permanent_redirect BOOL DEFAULT false,
...
mod tests {
	#[test]
	fn test_decode() {
		assert_eq!(decode(String::from("0")), 0);
		assert_eq!(decode(String::from("8m0Kx")), 123456789);
		assert_eq!(decode(String::from("15FTGf")), 999999999);
		assert_eq!(decode(String::from("2aL26ws")), 123456789012);
		assert_eq!(decode(String::from("LygHa16AHYF")), 9223372036854775807);
	}
}
```
